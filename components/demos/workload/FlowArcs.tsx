...
/** One source→edge-node connection for an assigned workload. */
...
export const LATENCY_GOOD_MS = 0.5;
export const LATENCY_BAD_MS = 5.5;
...
/**
 * Data paths for placed workloads: faint additive arc lines plus ONE
 * shared THREE.Points cloud of flowing packets (same idiom as Demo 02).
 * Per-particle progress advances in useFrame against a precomputed arc
 * sample table — pure Float32Array math, no allocations, no setState.
 * Packet speed falls with latency; colour encodes latency quality.
 */
export function FlowArcs({ streams }: FlowArcsProps) {
  ...
  const data = useMemo(() => {
    ...
    streams.forEach((s, i) => {
      ...
      // faint arc line, brightening source→node to hint at data direction
  ...
  // dispose superseded geometry (each re-assignment rebuilds it)
  useEffect(() => {
